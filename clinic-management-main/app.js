...
mongoose.connect("mongodb+srv://admin:hibro567@cluster0.en3fo.mongodb.net", {
...
app.get("/pdashboard", async (req, res) => {
  let unscheduled = await Consultation.find({
    status: "unscheduled",
    patientMobile: req.query.patientMobile,
  });
  let completed = await Consultation.find({
    status: "completed",
    patientMobile: req.query.patientMobile,
  });
  let patient = await Patient.findOne({ mobile: req.query.patientMobile });
  let patientCount = await Consultation.find({
    patientMobile: req.query.patientMobile,
...
app.get("/ddashboard", async (req, res) => {
  ...
  let totalConsultations = await Consultation.find({
    username: req.query.username,
  }).estimatedDocumentCount();
  let doctorCount = await Doctor.find({
    patientMobile: req.query.patientMobile,
  }).estimatedDocumentCount();
...
app.get("/list", async (req, res) => {
  ...
  if (doctor || patient || consultation) {
    res
      .status(200)
      .json({ doctor: doctor, patient: patient, consultation: consultation });
  }
});
...
app.get("/complete", async (req, res) => {
  ...
  res.redirect("ddashboard?username=" + req.query.username);
});
...
app.post("/bookConsultation", async (req, res) => {
  ...
  res.redirect("pdashboard?patientMobile=" + patientMobile);
});
